...
class LatLngPopup_Beta(MacroElement):
    """
    When one clicks on a Map that contains a LatLngPopup,
    a popup is shown that displays the latitude and longitude of the pointer.
    ...
    const getData = (e) => {
            ...
            {{this.get_name()}}
            .setContent(set_popup_content(responseData))
                .openOn({{this._parent.get_name()}});
            });}
    ...
    {{this._parent.get_name()}}.on('click', latLngget);
                {% endmacro %}
                """)  # noqa

    def __init__(self):
        super(LatLngPopup_Beta, self).__init__()
        self._name = 'LatLngPopup_Beta'
